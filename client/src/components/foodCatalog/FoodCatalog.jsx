...
const FoodCatalog = () => {
  ...
  useEffect(() => {
    const fetchFoodType = async () => {
      const res = await fetch(`https://foomania.onrender.com/product?category=${foodEndpoint}`, {
        headers: {
          ...
          {filteredFoods.length !== 0 ? filteredFoods.map((f) => (
            <Link to={`/food/${f._id}`} key={f._id} className={classes.food}>
              <div className={classes.imgContainer}>
                <img src={`https://foomania.onrender.com/images/${f?.img}`} className={classes.foodImg} />
              </div>
